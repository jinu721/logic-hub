...
export class UserController implements IUserController {
  ...
  updateCurrentUser = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!userData) throw new AppError(HttpStatus.BAD_REQUEST, "userData is required");

    const user = await this.commandSvc.updateUser(userId, dto);
    return sendSuccess(res, HttpStatus.OK, { user }, "User updated successfully");
  });

  getUsers = asyncHandler(async (req: Request, res: Response) => {
    const { search = "", page = "1", limit = "20" } = req.query as any;
    const result = await this.querySvc.findUsers(search, Number(page), Number(limit));
  ...
  toggleBan = asyncHandler(async (req: Request, res: Response) => {
    ...
    return sendSuccess(res, HttpStatus.OK, { result }, "User ban status toggled");
  });
  ...
  giftItem = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!userId || !type || !itemId) throw new AppError(HttpStatus.BAD_REQUEST, "Invalid inputs");

    const result = await this.engagementSvc.giftItem(userId, itemId, type);
    return sendSuccess(res, HttpStatus.OK, { result }, `Gifted ${type} successfully`);
  });
  ...
  verifyAdmin = asyncHandler(async (req: Request, res: Response) => {
    ...
    return sendSuccess(res, HttpStatus.OK, { message: "Admin verified", approved: true });
  });
}
