"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PendingUserRepository = void 0;
const user_1 = require("../../user");
const _core_1 = require("../../../shared/core");
class PendingUserRepository extends _core_1.BaseRepository {
    constructor() {
        super(user_1.pendingModel);
    }
    createPendingUser(pendingUser) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.create(pendingUser);
        });
    }
    findPendingUserByEmail(email) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.findOne({ email });
        });
    }
    deletePendingUser(email) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.deleteOne({ email });
        });
    }
    updatePendingUserOtp(email, otp) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.updateOne({ email }, { otp });
        });
    }
}
exports.PendingUserRepository = PendingUserRepository;
